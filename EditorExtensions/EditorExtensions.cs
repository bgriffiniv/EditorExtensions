...
namespace EditorExtensions
{
	...
	public class EditorExtensions : MonoBehaviour
	{
	    ...
	    public void Update()
	    {
			...
			// Tab - Toggle VAB/SPH editor mode (while staying in the same hangar)
	...
	*/
			...
			if (Input.GetKeyDown (KeyCode.C)) {
				...
				if (!altKeyPressed)
				{
					...
					float newAngle;
					if (shiftKeyPressed) {
						DebugMessage ("new AngleIndex: " + (currentAngleIndex == 0 ? angle.Length - 1 : currentAngleIndex - 1).ToString ());
        ...
        */
			...
			{
				if ((_symmetryMode < 1 && shiftKeyPressed))
				{
                    // If you go down past 0, go to max
                    _symmetryMode = maxSymmetryMode - 1;
				}
                else if (altKeyPressed || (_symmetryMode > maxSymmetryMode - 2 && !shiftKeyPressed))
				{
                    // If you go up past max (or if holding Alt), go to 0
					_symmetryMode = 0;
				}
				else
				{
